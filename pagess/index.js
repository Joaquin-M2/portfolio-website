...
import {
  promiseSingleTimeout,
  promiseTwoTimeouts,
} from '../utils/selfwriting-phrases';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const question =
      'Do you need a ninja-level web developer for your project?'.split('');

    const answerBeginningUntilComma = 'If so,'.split('');
    const answerBeginningAfterComma = ' have a look at '.split('');
    const myPortfolioSpan = (
      <Link
        legacyBehavior
        href='/portfolio/projects'
        key={'my portoflio button'}
      >
        <a>
          <ButtonFromNavBarBottom additionalStyles={styles.answerButtons}>
            my portfolio
          </ButtonFromNavBarBottom>
        </a>
      </Link>
    );
    const answerAnd = ' and '.split('');
    const dropMeALineSpan = (
      <Link legacyBehavior href='/contact' key={'drop me a line button'}>
        <a>
          <ButtonFromNavBarBottom additionalStyles={styles.answerButtons}>
            drop me a line
          </ButtonFromNavBarBottom>
    ...
    async function selfWritingText() {
      ...
      await promiseTwoTimeouts(
        question,
        questionWrittenCharacters,
        setQuestionContent,
        typingSpeed,
        questionLineTimeout,
        writeText
      ...
      await promiseTwoTimeouts(
        answerBeginningUntilComma,
        answerWrittenBeginningUntilComma,
        setAnswerContent,
        typingSpeed,
        1000,
        writeText
      ...
      await promiseTwoTimeouts(
        answerBeginningAfterComma,
        answerWrittenBeginningAfterComma,
        setAnswerContent,
        typingSpeed,
        waitingAfterComma,
        writeText
      );
      await promiseSingleTimeout(
        myPortfolioSpan,
        setAnswerContent,
        0,
        writeText
    ...
    return (
      <>
        ...
        <div className={styles.questionAnswer}>
          ...
          <p className={styles.answer}>{answerContent}</p>
        </div>
      </>
    );
  });
